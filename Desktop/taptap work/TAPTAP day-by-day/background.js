...
class Recorder {
  constructor(onBlobAvailable) {
    this.chunks = [];

    this.active = false;

    this.callback = onBlobAvailable;
  }
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (request === 'startTapTapRecording') {

    let rec = new Recorder((blob) => {
      
      console.log(blob[0].size*8/(128000+2500000))
      const videoUrl = URL.createObjectURL(blob[0]);
      console.log(videoUrl)
      chrome.downloads.download({url: videoUrl},()=>{
  ...
  } else if (request === 'stopTapTapRecording') {
    let rec = instance.get()
    rec.stop()
    // location.reload()
